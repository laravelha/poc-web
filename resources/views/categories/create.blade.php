        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h2>@lang('app.create.title', ['model' => __('categories.model')])</h2>
                </div>
                <div class="card-body">
                    <form action="{{ route('categories.store') }}" method="post" name="create">
                        @csrf
                        <div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
    <label for="title" class="control-label">@lang('categories.title')</label>
    <textarea id="title" class="form-control" name="title" maxlength="150" required autofocus>{{ old('title') ? old('title') : (isset($category->title) ? $category->title : '') }}</textarea>
    @if ($errors->has('title'))
...
@endpush
                        ...
                        <div class="form-group">
                            ...
                            <button type="button" class="btn btn-outline-dark" onclick="location.href='{{ route('categories.index') }}'">@lang('app.cancel')</button>
                        </div>
                    </form>
                </div>
            </div>
